...
cat > ~/cisco-rag-demo/workflows/document_upload_form.json << 'EOF'
{
  ...
  "nodes": [
    ...
    },
    {
      "parameters": {
        "method": "POST",
        "url": "=http://chromadb:8000/api/v1/collections/{{ $json.collection_uuid }}/add",
        "sendBody": true,
        "specifyBody": "json",
    ...
    },
    {
      "parameters": {
        "respondWith": "json",
        "responseBody": "={\n  \"success\": true,\n  \"message\": \"Document uploaded!\",\n  \"document_name\": \"{{ $('Parse Document Content').first().json.documentName }}\",\n  \"file_type\": \"{{ $('Parse Document Content').first().json.fileType }}\",\n  \"text_length\": {{ $('Parse Document Content').first().json.textLength }},\n  \"chunks_created\": {{ $('Process Document').all().length }}\n}"
      },
      "name": "Success",
  ...
  "connections": {
    ...
    "Store in ChromaDB": {"main": [[{"node": "Success", "type": "main", "index": 0}]]}
  },
...
**You should see:**
- 9 nodes in sequence
- Form Trigger → Extract → Parse → Get UUID → Process → Embed → Prepare → Store → Success
...
**In n8n:** All 9 nodes show green checkmarks
